...
for scan_root in folders_to_scan:
    ...
    for file_path in scan_root.rglob("*"):
        ...
        if config.client_data_dir in file_path.parents:
            source_group = "client_data"
            relative_to_group = file_path.relative_to(config.client_data_dir)
        elif config.corpus_dir in file_path.parents:
            source_group = "corpus_data"
            relative_to_group = file_path.relative_to(config.corpus_dir)
        else:
            source_group = "unknown"
            relative_to_group = file_path.relative_to(config.project_root)
...
# Print basic summary
print(f"Document inventory created successfully in path: {config.document_inv}")
